        ...
        <div class="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 relative z-10">
            <div class="flex flex-col justify-between h-full">
                <div>
                    ...
                    <h2 class="text-4xl md:text-6xl font-black font-syne leading-[0.9] mb-12 reveal-on-scroll"
                        v-html="t('footer.headline')"></h2>
                </div>

                <div class="hidden lg:block space-y-8 reveal-on-scroll delay-100">
                    <div class="flex flex-col gap-2">
                        <span class="text-sm uppercase opacity-60 font-bold tracking-widest">{{ t('footer.contactDetails') }}</span>
                        ...
                        <p class="text-2xl font-medium">{{ t('footer.location') }}</p>
                    </div>
            ...
            <div class="flex flex-col justify-center reveal-on-scroll delay-200">
                <form @submit.prevent="submitForm" class="space-y-8 md:space-y-12">
                    <div class="group relative">
                        <label class="block text-xs uppercase font-bold tracking-widest mb-2 opacity-60">{{ t('footer.nameLabel') }}</label>
                    ...
                    <div class="group relative">
                        <label class="block text-xs uppercase font-bold tracking-widest mb-2 opacity-60">{{ t('footer.emailLabel') }}</label>
                    ...
                    <div class="group relative">
                        <label class="block text-xs uppercase font-bold tracking-widest mb-2 opacity-60">{{
                            t('footer.messageLabel') }}</label>
                        <textarea v-model="form.message" required :placeholder="t('footer.messagePlaceholder')" rows="1"
                    ...
                    <button type="submit" :disabled="isSubmitting"
                        ...
                        <span v-else>{{ t('footer.sending') }}</span>
            ...
            <div class="lg:hidden mt-8 space-y-8 reveal-on-scroll">
                ...
                <div>
                    ...
                    <p class="text-lg">{{ t('footer.location') }}</p>
                </div>
...
const contactSchema = computed(() =>
    z.object({
        name: z.string().min(2, t('footer.validation.nameMin')),
        email: z.string().email(t('footer.validation.emailInvalid')),
        message: z.string().min(10, t('footer.validation.messageMin')),
    }),
)
...
const submitForm = async () => {
    ...
    const result = contactSchema.value.safeParse(form.value)
    if (!result.success) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(data.message || t('footer.errorSend'))
        }

        successMessage.value = t('footer.success')
        ...
        errorMessage.value = err.message || t('footer.errorConnection')
    } finally {
